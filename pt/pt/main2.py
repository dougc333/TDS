...
class Attrs:
    def __repr__(self):
        click.echo("class Attrs.__repr__")
...
def gen_config_impl(task_name, options):
    ...
    for opt in options:
        ...
        if found:
            eprint("INFO - Applying option:",
                   "->".join(reversed(found)), "=", opt)
            obj = root
...
def main(context, config_file, config_json, config_module, include):
    ...
    for path in include or []:
        print("path:", path)
        add_include(path)
    print("main 111111111")
    context.obj = Attrs()
    print("main 22222222222")
    # print("main path:",path)
    print("context", context.obj)
    print("main 333333333333")
        ...
        if not hasattr(context.obj, "config"):
            if config_module:
                ...
                if config_file:
                    ...
                    print("type json_input:", type(json_input))
                    # dict
                    config = json_input
                    print("config:", config)
                    print("type config:", type(config))
                    # parse_config should work on dict object
                print("parse_config coming")
                context.obj.config = parse_config(config)
                print("after parse_config")
                print("*********************")
                print(type(context.obj.config))
...
def gen_default_config(context, task_name, options):
    ...
    """
    print("main module func main gen_default_config")
    try:
...
def train(context):
    ...
    print("main train+++++++++++++++++++")
    config = context.obj.load_config()
    print("have config object")
    print("\n===sdfgsdgsdfgsdfgdsfStarting training...")
    metric_channels = []
...
def get_logits(context, model_snapshot, test_path, use_cuda, output_path, field_names):
    ...
    print("\n=== Starting get_logits...")
    workflow_get_logits(model_snapshot, use_cuda,
                        output_path, test_path, field_names)
