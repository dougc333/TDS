...
from .pt_config import PyTextConfig
...
def pytext_config_from_json(json_obj, ignore_fields=(), auto_upgrade=True):
    if auto_upgrade:
        print("pytext_config_from_json autoupgrade")
...
def config_from_json(cls, json_obj, ignore_fields=()):
    print("config_from_json cls:",cls)
    print("config_from_json json_obj:",json_obj)
    print("config_from_json ignore_fields:",ignore_fields)
    if getattr(cls, "__EXPANSIBLE__", False):
        print("config_from_json EXPANSIBLE")
        component_config = _try_component_config_from_json(cls, json_obj)
        if component_config:
            print("returning component_config")
            return component_config
    parsed_dict = {}
    print("creating parsed_dict")
    ...
    for field, f_cls in cls.__annotations__.items():
        ...
        if field not in json_obj:
            ...
            try:
                ...
                print("f_cls:",f_cls)
                print("json_obj[field]:",json_obj[field])
        ...
        print("parsed_dict[field]:",parsed_dict[field])
    return cls(**parsed_dict)


def _value_to_json(cls, value):
    print("_value_from_json, cls:",cls, " value:",value)
    cls_type = _get_class_type(cls)
...
def parse_config(config_json):
    ...
    """
    #config_json is sys.stdin representing docnn.json
    if "config" in config_json:
        config_json = config_json["config"]
    print("parse_config config_json:",config_json)
